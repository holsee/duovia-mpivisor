...
namespace DuoVia.Net
{
    internal sealed class ProxyFactory
    {
        public static TInterface CreateProxy<TInterface>(Type channelType, Type ctorArgType, object channelCtorValue) where TInterface : class
        {
            ...
            TypeBuilder typeBuilder = moduleBuilder.DefineType(typeof(TInterface).Name + "Proxy", typeAttributes, channelType);
            ...
            TInterface instance = (TInterface)t.GetConstructor(ctorArgTypes).Invoke(new object[] { channelCtorValue });
        ...
        private static MethodBuilder ConstructMethod(Type channelType, MethodInfo methodInfo, TypeBuilder typeBuilder, Dictionary<Type, OpCode> ldindOpCodeTypeMap, Dictionary<Type, OpCode> stindOpCodeTypeMap)
        {
            ...
            GenerateILCodeForMethod(channelType, mIL, parameterTypes, methodBuilder.ReturnType, ldindOpCodeTypeMap, stindOpCodeTypeMap);
            return methodBuilder;
        }

        private static void GenerateILCodeForMethod(Type channelType, ILGenerator mIL, Type[] inputArgTypes, Type returnType, Dictionary<Type, OpCode> ldindOpCodeTypeMap, Dictionary<Type, OpCode> stindOpCodeTypeMap)
        {
            int nofArgs = inputArgTypes.Length;
            //get the MethodInfo for InvokeMethod
            MethodInfo invokeMethodMI = channelType.GetMethod("InvokeMethod", BindingFlags.Instance | BindingFlags.NonPublic);
